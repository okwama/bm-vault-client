  ...
  const handleCreateTeam = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const teamMembers of teams) {
        // Find a crew commander from the team members (preferably someone with a leadership role)
        const crewCommander = teamMembers.find(member => 
        ...
        ) || teamMembers[0]; // Fallback to first member if no leadership role found

        await teamService.createTeam({
          name: `${teamName} ${teams.indexOf(teamMembers) + 1}`,
          members: teamMembers.map(member => member.id),
          crew_commander_id: crewCommander.id
        });
      }
