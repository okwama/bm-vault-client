...
import { ATM, getATMs, deleteATM } from '../services/atmService';
import serviceChargeService, { ServiceCharge } from '../services/serviceChargeService';
import { Building2, Plus, Pencil, Trash2, Truck, CreditCard } from 'lucide-react';
import BranchModal from '../components/Clients/BranchModal';
import ATMModal from '../components/Clients/ATMModal';
...
const ClientDetailsPage: React.FC = () => {
  ...
  const [atmFunctionalityAvailable, setAtmFunctionalityAvailable] = useState(true);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch client, branches, and service charges (required data)
      ...
      // Try to fetch ATMs (optional - don't fail if endpoint doesn't exist)
      try {
        const atmsData = await getATMs(id);
        setATMs(atmsData);
        setAtmFunctionalityAvailable(true);
      } catch (atmError) {
        console.warn('ATM endpoints not available yet:', atmError);
        setATMs([]); // Set empty array if ATMs are not available
        setAtmFunctionalityAvailable(false);
      }
  ...
  const handleEditATM = (atm: ATM) => {
    setSelectedATM(atm);
    setIsATMModalOpen(true);
  };

  const handleDeleteATM = async (atmId: string) => {
    if (!id) return;
    
    if (window.confirm('Are you sure you want to delete this ATM?')) {
      try {
        await deleteATM(id, atmId);
        setATMs(atms.filter(a => a.id !== atmId));
      } catch (err: any) {
        console.error('Error deleting ATM:', err);
        setError('ATM functionality not available yet. Please contact your administrator.');
      }
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mt-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-medium text-gray-900">ATMs</h2>
          <button
            onClick={handleAddATM}
            disabled={!atmFunctionalityAvailable}
            className={`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md ${
              atmFunctionalityAvailable 
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-md">
          {!atmFunctionalityAvailable ? (
            <div className="px-6 py-4 text-center">
              <p className="text-sm text-gray-500 mb-2">ATM functionality is not available yet.</p>
              <p className="text-xs text-gray-400">Please contact your administrator to enable this feature.</p>
            </div>
          ) : (
            <ul className="divide-y divide-gray-200">
              {atms.map(atm => (
                <li key={atm.id} className="px-6 py-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-900">{atm.atm_code}</p>
                      <p className="text-sm text-gray-500">{atm.location}</p>
                      {atm.comment && (
                        <p className="text-sm text-gray-400 mt-1">{atm.comment}</p>
                      )}
                    </div>
                    <div className="flex space-x-2">
                      <button
      ...
      {isATMModalOpen && (
        <ATMModal
          ...
          clientId={id || ''}
          atm={selectedATM || undefined}
          onSuccess={(atm) => {
            if (selectedATM) {
              setATMs(atms.map(a => a.id === atm.id ? atm : a));
            } else {
              setATMs([...atms, atm]);
            }
