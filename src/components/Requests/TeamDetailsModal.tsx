                  ...
                  {team ? (
                    <div>
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        {/* Crew Commander Section */}
                        {team.crew_commander && (
                          <div className="mb-4 p-3 bg-blue-50 rounded border-l-4 border-blue-400">
                            <p className="text-sm font-medium text-blue-900 mb-2">Crew Commander</p>
                            <div className="flex items-center space-x-3">
                              {team.crew_commander.photo_url && (
                                <img
                                  src={team.crew_commander.photo_url}
                                  alt={team.crew_commander.name}
                                  className="h-8 w-8 rounded-full object-cover"
                                />
                              )}
                              <div>
                                <p className="text-sm font-medium text-blue-900">{team.crew_commander.name}</p>
                                <p className="text-sm text-blue-700">{team.crew_commander.role}</p>
                              </div>
                            </div>
                          </div>
                        )}
                          ...
                          <div className="space-y-3">
                            {team.members.map((member) => (
                              <div key={member.id} className="flex items-center space-x-3">
                                ...
                                {team.crew_commander && member.id === team.crew_commander.id && (
